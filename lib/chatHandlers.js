...
exports.registerChatHandlers = (io, socket) => {
    ...
    const getChats = async () => {
        try {
            ...
            const chats1 = await Chat.find({firstUser}).populate("firstUser secondUser").lean();
            const chats2 = await Chat.find({secondUser: firstUser}).populate("firstUser secondUser").lean();
